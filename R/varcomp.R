...
varcomp <- function(model, ci = TRUE, level = 0.95) {
  # vcov_lmerMod(model, full = TRUE, ranpar = "var"),
  # vcov_lmerMod(model, full = TRUE, ranpar = "sd")?
  if (any(
    ...
    if (inherits(model, "glmerMod")) {
      random_vcov <-
        as.matrix(vcov_glmerMod(model, full = TRUE,
                                ranpar = "var")[vcov_index, vcov_index])
      ci <- FALSE
    } else {
      random_vcov <-
        as.matrix(vcov_lmerMod(model, full = TRUE,
                               ranpar = "var")[vcov_index, vcov_index])
    }
    ...
    if (ci) {
      varcomp <- cbind(varcomp, confint(model,
                                        level = level)[seq_len(random_terms), ])
      kk <- abs(varcomp$sdcor^2 - varcomp$vcov) > 0.0000000000001
      varcomp[, 7] <- ifelse(!kk,
                             ifelse(varcomp[, 7] < 0, 0, varcomp[, 7]^2),
                             varcomp[, 7] * varcomp$vcov / varcomp$sdcor)
      varcomp[, 8] <- ifelse(!kk, varcomp[, 8]^2, varcomp[, 8] * varcomp$vcov /
                               varcomp$sdcor)
    ...
    confint_list <- try(intervals(model, which = "var-cov", level = level),
                        silent = TRUE)
    if (!inherits(confint_list, "try-error")) {
      confint_matrix <- as.matrix(rbind(do.call("rbind", confint_list[[1]]),
                                        confint_list$sigma))
      confint_names <- c(as.vector(sapply(confint_list[[1]], rownames)),
                         "residual")
      ...
      if (any(cor_detect)) {
        cor_names <- sub("\\)$", "",
                         sub("^cor\\(", "", confint_names[cor_detect]))
        sd_value <- confint_matrix[sd_detect, "est."]
        names(sd_value) <- sd_names
        sd_prod <- sapply(strsplit(cor_names, ","), function(x) {
          sdv <- sd_value[x]
          sdv[1] * sdv[2]
        })
        # print(sd_prod)
        # print(confint_matrixN[cor_detect, ])
        confint_matrixN[cor_detect, ] <- confint_matrix[cor_detect, ] *
          sd_prod
      }
      rownames(confint_matrixN) <-
        as.character(round(confint_matrixN[, "est."], 4))
    ...
    theta_UL <- theta + qnorm(p = (1 - level) / 2, lower.tail = FALSE) *
      theta_sd
    upper <- theta_sd * 2 * exp(2 * theta_UL)
    theta_LL <- theta - qnorm(p = (1 - level) / 2, lower.tail = FALSE) *
      theta_sd
