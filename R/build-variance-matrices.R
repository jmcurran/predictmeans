  ...
  if (is.null(R_list)) {
    ...
    if (is.null(mod$groups)) {
      ...
      if (is.null(mod$modelStruct$varStruct)) {
        ...
        sd_vec <- sigma / as.numeric(varWeights(mod$modelStruct$varStruct))
      ...
      if (is.null(mod$modelStruct$varStruct)) {
        grps <- mod$groups[[1]]
        V_list <- tapply(rep(sigma^2, length(grps)), grps, function(x) {
        ...
        V_list <- tapply(sd_vec^2, mod$groups[[1]], function(x) {
          diag(x, nrow = length(x))
        })
      }
...
ZDZt <- function(D, Z_list) {
  lapply(Z_list, function(z) {
    z %*% D %*% t(z)
  })
}

## Construct list of block-diagonal matrices for each random effects grouping
## structure
...
build_RE_mats <- function(mod, sigma_scale = FALSE) {
  ...
  if (length(all_groups) == 1) {
    ...
    if (sigma_scale) {
      D_list <- lapply(mod$modelStruct$reStruct, function(x) {
        mod$sigma^2 * as.matrix(x)
      })
    } else {
    ...
    data <- nlme::getData(mod)
    Z_mat <- model.matrix(mod$modelStruct$reStruc, data[complete.cases(data), ])
    Z_names <- sapply(strsplit(colnames(Z_mat), ".", fixed = TRUE),
                      function(x) {
                        x[1]
                      })
    row.names(Z_mat) <- NULL
    Z_levels <- lapply(names(all_groups), function(x) {
      Z_mat[, x == Z_names, drop = FALSE]
    })
    Z_levels <- Map(matrix_list,
                    x = Z_levels,
                    fac = all_groups,
                    dim = "row")
...
#' @export
#' @keywords internal
build_Sigma_mats.default <- function(mod,
                                     invert = FALSE,
                                     sigma_scale = FALSE) {
  mod_class <- paste(class(mod), collapse = "-")
  stop(paste0(
    "Sigma matrices not available for models of class ",
    mod_class,
    "."
  ))
}
...
#' @export
#' @keywords internal
build_Sigma_mats.gls <- function(mod,
                                 invert = FALSE,
                                 sigma_scale = FALSE) {
  ...
  if (invert) {
    V_list <- lapply(V_list, function(x) {
      chol2inv(chol(x))
    })
...
#' @export
#' @keywords internal
build_Sigma_mats.lme <- function(mod,
                                 invert = FALSE,
                                 sigma_scale = FALSE) {
  ...
  group_mapping <- tapply(ZDZ_grps, V_grps, function(x) {
    length(unique(x))
  })
  ...
  if (invert) {
    Sigma_list <- lapply(Sigma_list, function(x) {
      chol2inv(chol(x))
    })
  }
