...
mean_plot <- function(plot_mt, x_var, y_var, col_var = NULL, panel_var = NULL, title = NULL, xlab = NULL, ylab = NULL,
                      scales = "fixed", bar_value = 0, bar_label = c("Aveg.LSD", "Aveg.SED"), level = 0.05, basesz = 12,
                      line = TRUE) {
  ...
  if (!is.null(x_var) && is.null(col_var) && is.null(panel_var)) {
    if (is.null(title) || any(c("NULL", "") == title)) {
      if (bar_value > 0) {
        title <- paste("Predicted means for '", ylab, "' vs '", xlab, "' with ", bar_label, " Bar at ", level,
                       " Significant Level\n", sep = "")
      } else {
  ...
  if (!is.null(x_var) && !is.null(col_var) && is.null(panel_var)) {
    if (is.null(title) || any(c("NULL", "") == title)) {
      if (bar_value > 0) {
        title <- paste("Predicted means for '", ylab, "' vs '", xlab, "' by '", col_var, "' with ", bar_label,
                       " Bar at ", level, " Significant Level\n", sep = "")
      } else {
    ...
    meanPlot <- ggplot(plot_mt, aes(.data[[x_var]], .data[[y_var]], group = .data[[col_var]], col = .data[[col_var]])) +
      ...
      theme_bw(basesz)
    if (line) {
      meanPlot <- meanPlot + geom_line(aes(
        linetype = .data[[col_var]],
        col = .data[[col_var]]
      ), linewidth = 0.96) +
  ...
  if (!is.null(x_var) && !is.null(col_var) && !is.null(panel_var)) {
    ...
    meanPlot <- ggplot(plot_mt, aes(.data[[x_var]], .data[[y_var]],
      ...
      labs(title = paste(title, "\n", sep = ""), x = paste("\n", xlab, sep = ""), y = paste(ylab, "\n", sep = "")) +
      lims(x = levels(plot_mt[, x_var]), y = c(yMin - 0.5 * offSet, max(yMax + 0.5 * offSet,
                                                                        yMin + bar_value + 0.5 * offSet))) +
      geom_errorbar(aes(ymax = barMax, ymin = barMin, x = bar_label), width = 0.15, linewidth = 0.8, colour = "blue") +
      geom_point(size = 2) +
