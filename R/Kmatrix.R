...
#' @importFrom stats model.matrix na.pass terms
#'
...
Kmatrix <- function(model,
  ...
  if (inherits(model, "mer") ||
        ...
        inherits(model, "lme") || inherits(model, "gls")) {
  ...
  if ((!is.null(covariate) && !unique(covariate %in% c("NULL", ""))) &&
    ...
    if (length(covariate) == 1) {
      if ((!is.null(covariateV) && !unique(covariateV %in% c("NULL", ""))) &&
            is.vector(covariateV)) {
      ...
      if ((!is.null(covariateV) && !unique(covariateV %in% c("NULL", ""))) &&
        ...
        for (i in covariate) {
          baselevs[[i]] <- sort(unique(c(seq(
            min(X[[i]]), max(X[[i]]), length = 30
          ))))
        }
      }
    }
  ...
  ## OK. Now make a grid of the factor levels of interest, along w/
  ## covariate "at" values
  grid <- do.call("expand.grid", baselevs)

  ## add any matrices
  for (nm in names(matdat)){
    grid[[nm]] <- matrix(rep(matdat[[nm]], each = nrow(grid)),
                         nrow = nrow(grid))
  }

  ## Now make a new dataset with just the factor combs and covariate values we
  ## want for prediction WARNING -- This will overwrite X, so get anything you
  ## need from X BEFORE we get here
  if (!is.null(ndf)) {
  ...
  if (!setequal(covariate, modelterm) &&
        max_ord > 1 &&
        length(factor_names) > 1  &&
        any(ndf$Freq == 0) &&
        !(setequal(unique(ndf$Freq), c(0, 1)))) {

    warning(paste0("Missing treatments' combination appeared, predicted means",
                   "maybe misleading!"))
    X[ndf$Freq == 0, ] <- NA
  }
  ...
  if (any(sapply(facs, function(nm) {
  ...
  fctnames <- cbind(fctnames, K[, setdiff(covlevname, names(fctnames)), drop =
                                  FALSE])
