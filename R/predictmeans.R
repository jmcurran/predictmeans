...
#' @param lsd_bar A logical variable to indicate to print an average LSD or SED
#' bar on the means plot. The default is TRUE.
...
#' interactions.
#' @param lineplot An option for drawing a line chart, or dot chart. The
#' default is TRUE.
#' @param plottitle A character vector specifying the main title for plot(s).
#' The default is NULL.
#' @param plotxlab A character vector specifying the x label for plot(s). The
#' default is NULL.
#' @param plotylab A character vector specifying the y label for plot(s). The
#' default is NULL.
#' @param mplot An option for drawing a means plot, or not. The default is
#' TRUE.
#' @param barplot An option for drawing a bar chart, or not. The default is
#' FALSE.
#' @param pplot An option for drawing a p-values plot, or not when there are
#' more than six p-values. The default is TRUE.
#' @param bkplot An option for drawing back transformed plot, or not. The
#' default is TRUE.
#' @param plot An option for drawing plots, or not. The default is TRUE.
#' @param jitterv A degree of jitter in x and y direction in the back
#' transformed means graph. The default is zero.
#' @param basesz The base font size. The default is 12.
#' @param prtnum An option for printing covariate information on the screen, or
#' not. The default is TRUE.
#' @param prtplt An option for printing plots on the screen, or not. The
#' default is TRUE.
...
#' means with CIs are also shown.} \item{predictmeansPlot}{ggplot of predicted
#' means.} \item{predictmeansBKPlot}{ggplot of back transformed means.}
...
#' @importFrom ggplot2 ggplot geom_bar geom_errorbar geom_errorbarh geom_line
#' @importFrom ggplot2 geom_point guides guide_legend
#' @importFrom ggplot2 labs lims position_dodge position_jitter
#' @importFrom ggplot2 scale_fill_brewer scale_y_discrete theme theme_bw
...
#' @importFrom parallel clusterExport makeCluster mclapply parLapplyLB
...
predictmeans <- function(model,
  ...
  if (!(modelterm %in% attr(terms(model), "term.labels"))) {
    stop(
      paste(
        "The",
        modelterm,
        paste0("must be exactly a term in the model (especially check the",
  ...
  if (all.null(mplot, barplot, pplot, bkplot)) {
    which.plots <- match.arg(
      which.plots,
      choices = c("mean", "bar", "pval", "back"),
      several.ok = TRUE
    )
    ...
    ## Once you remove the old arguments you remove the if statement and delete
    ## this
    ...
    warnMessage <- paste0(
      ...
      " ",
      verb,
      paste0(" deprecated. Use which.plots in the future. The value will be",
             " honored for now, but may not always be.")
    )
    warning(warnMessage)
  }
  ...
  if (!is.null(plotord) && !unique(plotord %in% c("NULL", ""))) {
    ...
    if (is.character(plotord)) {
      ...
      if (any(is.na(i))) {
        stop(paste(
          "The entries",
          paste0(plotord[is.na(i)], sep = ", "),
          "do not match any term in ",
          modelterm
        ))
      } else {
        plotord <- i
  ...
  mean.table <- round(xtabs(pm ~ ., mt[, c("pm", vars)],
                            drop.unused.levels = TRUE), ndecimal)
  se.table <- round(xtabs(ses ~ ., mt[, c("ses", vars)],
                          drop.unused.levels = TRUE), ndecimal + 1)
  ...
  if (nK == 1) {
    ...
    for (i in 2:nK) {
      # To construct pairwise comparison K matrix by col order
      for (j in 1:(i - 1)) {
    ...
    KKvarndiff <- data.frame(
      matrix(
        unlist(strsplit(KKvarn1, "\\:")),
        byrow = TRUE,
        nrow = length(KKvarn1)
      ),
      matrix(
        unlist(strsplit(KKvarn2, "\\:")),
        byrow = TRUE,
        nrow = length(KKvarn2)
      )
    )
    ...
    dses.df <- data.frame(
      matrix(
        unlist(strsplit(varn1, "\\:")),
        byrow = TRUE,
        nrow = length(varn1)
      ),
      matrix(
        unlist(strsplit(varn2, "\\:")),
        byrow = TRUE,
        nrow = length(varn2)
      ),
      dses
    ...
    if (length(vars) > 1) {
      ...
      for (i in seq_along(vars)) {
        varsindx <- as.character(dses.df[, i]) ==
          as.character(dses.df[, length(vars) + i])
    ...
    if (is.null(permlist) || all(permlist %in% c("NULL", ""))) {
      ...
      if (length(vars) > 1) {
        ...
        attr(LSD, "For the Same Level of Factor") <-
          round(qt(1 - level / 2, df = Df) * dses.m, ndecimal + 1)
      }  # end of if LSD
      ...
      if (length(vars) > 1) {
        rownames(dses.m) <- c("Aveg.LSD", "Min.LSD", "Max.LSD")
        attr(LSD, "For the Same Level of Factor") <- round(2 * dses.m,
                                                           ndecimal + 1)
      }

      attr(LSD, "Note") <- paste0("This is a approximate LSD (i.e. 2*SED)",
                                  "at 0.05 level.")
    }
    ...
    if (pairwise) {
      ...
      if (all(is.null(permlist) ||
                all(permlist %in% c("NULL", "")), adj == "tukey")) {
      ...
      if (is.null(permlist) ||
            all(permlist %in% c("NULL", ""))) {
        if (inherits(model, "lmerMod")) {
          t.p.values <- sapply(seq_along(t.v), function(m) {
            2 * pt(-abs(t.v[m]), Df_diff[m])
          })
        } else {
        ...
        if (.Platform$OS.type == "windows") {
          ...
          t.TableL <- parLapplyLB(cl, permlist[[1]], function(x) {
      ...
      if (is.null(atvar) || all(atvar %in% c("NULL", ""))) {
        ...
        if (is.null(permlist) ||
          ...
          attr(t.p.valuem, "Note") <- paste(
            "The matrix has t-value above the diagonal, p-value (adjusted by '",
            adj,
            "' method) below the diagonal",
            sep = ""
          )
          ...
          attr(LSDm, "Note") <- paste(
            paste0("LSDs matrix has mean differences (row-col) above the ",
                   "diagonal, LSDs (adjusted by '"),
            adj,
            "' method) below the diagonal",
            sep = ""
          )
          ...
          attr(t.p.valuem, "Note") <- paste(
            "The matrix has t-value above the diagonal, and ",
            nsim,
            " times permutation p-value (adjusted by '",
            adj,
            "' method) below the diagonal",
            sep = ""
          )
        ...
        t.p.valuemGrp[upper.tri(t.p.valuemGrp)] <-
          t(t.p.valuemGrp)[upper.tri(t.p.valuemGrp)]
        ...
        if (all(nrow(t.p.valuep) > 2, pplot, plot, prtplt)) {
          ...
          if (is.null(plottitle) ||
                plottitle %in% c("NULL", "")) {
            mtitle <- paste(
              "Level Plot of p-value (adjusted by '",
              adj,
        ...
        for (i in which(vars %in% atvar)) {
          # To find rows relating atvar
          KKvarndiff <-
            KKvarndiff[which(as.character(KKvarndiff[, i]) ==
                               as.character(KKvarndiff[, length(vars) + i])), ]
        }

        atvar.df <- f_loj_krc(
          KKvarndiff,
          dses.df,
          by.x = names(KKvarndiff),
          by.y = names(KKvarndiff)
        )
        names(atvar.df)[seq_along(vars)] <- vars

        for (i in vars) {
          # To ensure factor vars have the same level as original
          atvar.df[, i] <- factor(atvar.df[, i], levels = levels(mdf[, i][[1]]))
        }

        # atvar.df$adj.pvalue <- unlist(lapply(split(atvar.df,
        #                                            atvar.df[, atvar]),
        # function(x) {
        #   t_value <- x$tvalue
        ...
        #   } else {
        #     if (adj == "tukey") {
        #       ## Df need to be updated
        #       x$adj.pvalue <- ptukey(sqrt(2) * abs(x$tvalue),
        #                              t_valueN,
        #                              Df,
        #                              lower.tail = FALSE)
        #     } else {
        ...
        #   }
        # }))
        ## DONGWEN: I have used ChatGPT to rewrite the anonymous function.
        ## Hope this still works!

        ## Define function to adjust p-values within each group
        adjust_pvalues <- function(group) {
          t_values <- group$tvalue
          valid_n <- sum(!is.na(t_values))
          ...
          if (adj == "tukey") {
            return(ptukey(sqrt(2) * abs(t_values), valid_n, Df,
                          lower.tail = FALSE))
          } else {
            return(p.adjust(group$pvalue, method = adj))
          }
        }

        ## Apply adjustment function by group
        atvar.df$adj.pvalue <- unlist(
          lapply(split(atvar.df, atvar.df[[atvar]]), adjust_pvalues)
        )

        ## To find the suitable names for pm
        rnK.df <- as.data.frame(matrix(unlist(strsplit(rnKK, "\\:")),
                                       byrow = TRUE, nrow = nKK))
        ...
        ## The rest of vars rather than at var
        resvar <- vars[!vars %in% atvar]
        ## To ensure the right matrix name later
        rnK.df <- rnK.df[, c(atvar, resvar)]
        atvar.levels <- unique(do.call("paste",
                                       c(rnK.df[, atvar, drop = FALSE],
                                         sep = " : ")))
        resvar.levels <- unique(do.call("paste",
                                        c(rnK.df[, resvar, drop = FALSE],
                                          sep = " : ")))
        ## row col names of image plot
        rcnplotm <- do.call("paste", c(rnK.df[, , drop = FALSE], sep = " : "))
        ...
        for (i in 1:listlength) {
          # extract pvalues for each factor level
          atvar.pm <- matrix(0, nrow = nrow.pm, ncol = nrow.pm)
          atvar.pm[upper.tri(atvar.pm)] <-
            atvar.df$adj.pvalue[(indexlength * i -
                                 (indexlength - 1)):(indexlength * i)]
          rownames(atvar.pm) <- colnames(atvar.pm) <- resvar.levels
          pmlist[[i]] <- t(atvar.pm)
          outtab <- round(as.table(t(atvar.pm)), 4)
          outtab[outtab < 1e-04] <- "0.0001"
          outtab[col(outtab) == row(outtab)] <- 1
          outtab[upper.tri(outtab)] <- ""
          Grpatvar.pm <- t(atvar.pm) + atvar.pm

          if (all(!is.na(outtab))) {
            Grpatvar.letter <-
              multcompLetters(Grpatvar.pm,
                              Letters = LETTERS,
                              threshold = slevel)[resvar.levels]
            ...
            Grpatvar.letterM[atvar.rowname] <-
              multcompLetters(Grpatvar.pm[atvar.rowname, atvar.rowname],
                              Letters = LETTERS,
                              threshold = slevel)[atvar.rowname]
    ...
    alpha <- 1 - slevel / 2
    if (is.null(permlist) || all(permlist %in% c("NULL", ""))) {
      tTimesSE <- qt(alpha, df = meanTable$Df) * meanTable$ses
      meanTable$LL <- meanTable$pm - tTimesSE
      meanTable$UL <- meanTable$pm + tTimesSE
    } else {
    ...
    meanTable[c("pm", "LL", "UL")] <- round(meanTable[c("pm", "LL", "UL")],
                                            ndecimal)
    ...
    if (plot) {
      ...
      if (is.null(atvar) || all(atvar %in% c("NULL", ""))) {
        ...
        if (lsd_bar) {
          ...
          LSD_value <- mean(attr(SED.out,
                                 "For the Same Level of Factor")[1, atvar])
        }
      }
      ...
      limits <- aes(
        ymax = (pm + ses) * (pm > 0) + pmin(pm + ses, 0) * (pm <= 0),
        ymin = (pm - ses) * (pm < 0) + pmax(pm - ses, 0) * (pm >= 0)
      )
      ...
      if (length(vars) == 1) {
        ...
        if (mplot) {
          ...
          p1 <- ggplot(plotmt, aes(eval(parse(text = vars)), pm, group = 1)) +
            ...
            lims(
              x = c(bar_label, levels(plotmt[, vars])),
              y = c(yMin - offSet,
                    max(yMax + offSet, yMin + LSD_value + offSet))
            ) +
        ...
        if (barplot) {
          ...
          if (is.null(plottitle) ||
                plottitle %in% c("NULL", "")) {
            mtitle <- paste("Predicted means for \"",
                            modelterm,
                            "\" with SE Bars",
                            sep = "")
          }
          ...
          bp1 <- ggplot(plotmt, aes(eval(parse(text = vars)), pm)) + labs(
            title = paste(mtitle, "\n", sep = ""),
            x = mxlab,
            y = mylab
          ) + ylim(c(min(0, (
        ...
        ## NB: I have changed this to an else statement because there shouldn't
        ## be any other way to get here. That is, length(vars) will not change
        ## from 1 to 2 between the block where length(vars)==1 is true and here,
        ## therefore the cases should be else if statements.
        if (is.null(plotord) ||
              all(plotord %in% c("NULL", ""))) {
          plotord <- 1:2

          if (!(is.null(atvar) ||
                  all(atvar %in% c("NULL", "")))) {
            atvar_num <- which(vars %in% atvar)
            plotord <- c(atvar_num, setdiff(seq_along(vars), atvar_num))
          }
        }
        ...
        if (mplot) {
          ...
          plotmt[, fact1] <- factor(plotmt[, fact1],
                                    levels = c(bar_label,
                                               levels(plotmt[, fact1])))
          p2 <- ggplot(plotmt,
                       aes(x = eval(parse(text = fact1)),
                           y = pm,
                           group = eval(parse(text = fact2)),
                           col = eval(parse(text = fact2)))) +
            labs(title = paste(mtitle, "\n", sep = ""),
              x = mxlab,
              y = mylab
            ) +
            lims(x = levels(plotmt[, fact1]),
              y = c(yMin - offSet, max(yMax + offSet, yMin +
                                         LSD_value + offSet))
            ) +
            geom_point(size = 2) +
            geom_errorbar(
          ...
          if (lineplot) {
            p2 <- p2 +
              geom_line(aes(
                linetype = eval(parse(text = fact2)),
                col = eval(parse(text = fact2))
              ), linewidth = 0.96) +
        ...
        if (barplot) {
          ...
          bp2 <- ggplot(plotmt, aes(
            eval(parse(text = fact1)),
            pm,
            group = eval(parse(text = fact2)),
            fill = eval(parse(text = fact2))
          )) +
            geom_point(position = dodge) +
            geom_bar(stat = "identity", position = dodge) +
            labs(x = mxlab, y = mylab, title = mtitle) +
            ylim(c(min(0, (min(pm) - max(ses)) * 1.1),
                   max(0, (max(pm) + max(ses)) * 1.1))) +
            geom_errorbar(
              limits,
              position = dodge,
              width = 0.25,
            ...
            guides(fill = guide_legend(title = fact2))
        ...
        ## NB: I have changed this to else if, because neither length(vars) or
        ## mplot should have changed, and I believe this is just the third
        ## choice
        if (newwd) {
          dev.new()
        }
        ...
        if (is.null(plotord) ||
          ...
          if (!(is.null(atvar) ||
                  all(atvar %in% c("NULL", ""))) && length(atvar) == 1) {
            atvar_num <- which(vars %in% atvar)
            plotord <- c(atvar_num, setdiff(seq_along(vars), atvar_num))
          }
        }
        ...
        p3 <- ggplot(plotmt,
          aes(x = eval(parse(text = fact1)),
              y = pm,
              group = factor(eval(parse(text = fact2))),
              col = factor(eval(parse(text = fact2))))
        ) +
          labs(
            title = paste(mtitle, "\n", sep = ""),
            x = mxlab,
          ...
          lims(
            x = levels(plotmt[, fact1]),
            y = c(yMin - 0.5 * offSet,
                  max(yMax + 0.5 * offSet,
                      yMin + LSD_value + 0.5 * offSet))
          ) +
          geom_errorbar(
            aes(
              ymax = up,
        ...
        if (lineplot) {
          p3 <- p3 + geom_line(aes(
            linetype = eval(parse(text = fact2)),
            col = eval(parse(text = fact2))
          ), linewidth = 0.8) + guides(linetype = guide_legend(title = fact2))
        }
  ...
  if (!is.null(trans)) {
    ...
    if (identical(trans, make.link("log")$linkinv) ||
          identical(trans, exp)) {
      bkmt$Mean <- exp(bkmt$pm) - transOff
    }

    if (is.null(permlist) || all(permlist %in% c("NULL", ""))) {
      tTimesSE <- qt(1 - slevel * 0.5, df = bkmt$Df) * bkmt$ses
      bkmt$LL <- trans(bkmt$pm - tTimesSE) - transOff
      bkmt$UL <- trans(bkmt$pm + tTimesSE) - transOff
    } else {
    ...
    if (plot && bkplot) {
      ...
      if (response %in% names(mdf)) {
        ## Transformed y before modelling
        ...
        ## Transformed y within modelling
        nresponse <- regmatches(response,
                                regexec("\\(([^<]+)\\)", response))[[1]][2]

        if (!(nresponse %in% names(mdf))) {
          if (is.null(responsen) || all(responsen %in% c("NULL", ""))) {
            stop(
              paste(
                paste0("Please provide suitable name for response variable ",
                       "using option responsen='"),
                names(mdf)[1],
                "'!",
                sep = ""
              )
      ...
      if (is.null(atvar) || all(atvar %in% c("NULL", ""))) {
        ...
        Trtn <- do.call("paste",
                        c(mdf[, c(atvar, resvar), drop = FALSE], sep = ":"))
        newdata2 <- data.frame(bky = bky, Trtn = Trtn)
        bkmt$Trt <- do.call("paste",
                            c(bkmt[, c(atvar, resvar), drop = FALSE],
                              sep = ":"))
      }
      ...
      bkmt[, c("Mean", "LL", "UL")] <- lapply(bkmt[, c("Mean", "LL", "UL")],
                                              as.numeric)
    ...
    if (!(is.null(atvar) || all(atvar %in% c("NULL", "")))) {
      ...
      # meanTable <- with(meanTable,
      #                   meanTable[order(eval(parse(
      # text = c(atvar, resvar)[length(vars):1]))), ])

      ## DONGWEN: Have trupied to simplify the commented code above
      ## General version for any number of variables
      order_vars <- c(atvar, resvar)[rev(seq_along(vars))]
      meanTable <- meanTable[do.call(order, meanTable[order_vars]), ]

    }

    # if (all(!identical(trans, function(x) x, ignore.environment=TRUE),
    # !identical(trans, I,
    # ignore.environment=TRUE))) {
    if (any(meanTable$Mean != bkmt$Mean)) {
      meanTable <- cbind(meanTable,
                         round(bkmt[, (ncol(bkmt) - 2):ncol(bkmt)], ndecimal))
      colnames(meanTable)[(ncol(meanTable) - 2):ncol(meanTable)] <- c(
        "Bk_Mean",
        paste("Bk_LL(", (1 - slevel) *
                100, "%)", sep = ""),
    ...
    if (pairwise) {
      if ((is.null(atvar) || all(atvar %in% c("NULL", "")))) {
        ...
        meanTable$LetterGrp <- multcompLetters(t.p.valuemGrp,
                                               Letters = LETTERS,
                                               threshold = slevel)

        if (letterCI) {
          meanTable$LetterGrp <-
            ci_mcp(meanTable[, grepl("^LL", names(meanTable))],
        ...
        meanTable <- list(
          Table = meanTable,
          Note = paste(
            paste0("Letter-based representation of pairwise comparisons ",
        ...
        ## attr(meanTable, 'Note') <- paste(
        # 'Note: letter-based representation of pairwise
        ## comparisons at significant level '', slevel, ''', sep='')
      } else {
        meanTable$LetterGrp <- unlist(pmlistLetter)
        meanTable <- list(
          Table = meanTable,
          Note = paste(
            paste0("Letter-based representation of pairwise comparisons at ",
                   "significant level '"),
            slevel,
            "' at each level of ",
            atvar,
  ...
  if (pairwise) {
    if (is.null(atvar) || all(atvar %in% c("NULL", ""))) {
      if (all(is.null(permlist) ||
                all(permlist %in% c("NULL", "")),
              adj %in% c("none", "bonferroni"))) {
        outputlist <- list(
          `Predicted Means` = mean.table,
          `Standard Error of Means` = se.table,
          `Standard Error of Differences` = SED.out,
          LSD = LSD,
          `Pairwise LSDs` = round(LSDm, ndecimal + 1),
          `Pairwise p-value` = round(t.p.valuem, 4),
          predictmeansPlot = predictmeansPlot,
          predictmeansBarPlot = predictmeansBarPlot,
          mean_table = meanTable,
      ...
      if (is.null(permlist) ||
        ...
        names(outputlist) <- c(
          ...
          "LSD",
          paste(
            "Pairwise comparison p-value (adjusted by '",
            adj,
            "' method)",
            sep = ""
          ),
        ...
        names(outputlist) <- c(
          ...
          paste(
            "Pairwise '",
            nsim,
            "' times permuted p-value (adjusted by '",
            adj,
            "' method)",
            sep = ""
          ),
