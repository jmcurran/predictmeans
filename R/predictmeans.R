...
#' @export
predictmeans <- function(model, modelterm, data = NULL, pairwise = FALSE, atvar = NULL, adj = "none", Df = NULL,
    lsd_bar = TRUE, level = NULL, covariate = NULL, meandecr = NULL, letterCI = FALSE, trans = I, transOff = 0,
    responsen = NULL, count = FALSE, plotord = NULL, lineplot = TRUE, plottitle = NULL, plotxlab = NULL, plotylab = NULL,
    which.plots = c("mean", "pval", "back"), mplot = NULL, barplot = NULL, pplot = NULL, bkplot = NULL, plot = TRUE,
    jitterv = 0.2, basesz = 12, prtnum = TRUE, prtplt = TRUE, newwd = FALSE, permlist = NULL, ncore = 3, ndecimal = 4) {
    ...
    if (nK == 1) {
        ...
        if (pairwise) {
          ...
          if (all(is.null(permlist) || all(permlist %in% c("NULL", "")), adj == "tukey")) {
            if (inherits(model, "lmerMod")) {
              p.tukey <- sapply(1:length(t.v), function(m) ptukey(sqrt(2) * abs(t.v[m]), nK, Df_diff[m],
                lower.tail = FALSE))
            } else {
          ...
          if (is.null(permlist) || all(permlist %in% c("NULL", ""))) {
            ...
            if (.Platform$OS.type == "windows") {
              ...
              t.TableL <- parLapplyLB(cl, permlist[[1]], function(x) round(abs(tValue(x, rK)), 6) >= round(abs(t.v),
                6))
              stopCluster(cl)
            } else {
          ...
          if (is.null(atvar) || all(atvar %in% c("NULL", ""))) {
            ...
            if (is.null(permlist) || all(permlist %in% c("NULL", ""))) {
              ...
              attr(t.p.valuem, "Note") <- paste("The matrix has t-value above the diagonal, and ", nsim,
                " times permutation p-value (adjusted by '", adj, "' method) below the diagonal", sep = "")
            }  # end of if (is.null(permlist))
            ...
            for (i in which(vars %in% atvar)) {
              ...
              KKvarndiff <- KKvarndiff[which(as.character(KKvarndiff[, i]) == as.character(KKvarndiff[,
                length(vars) + i])), ]
            }
            ...
            for (i in 1:listlength) {
              ...
              atvar.pm[upper.tri(atvar.pm)] <- atvar.df$adj.pvalue[(indexlength * i - (indexlength - 1)):(indexlength *
                i)]
        ...
        if (plot) {
            ...
            limits <- aes(ymax = (pm + ses) * (pm > 0) + pmin(pm + ses, 0) * (pm <= 0), ymin = (pm - ses) * (pm <
                0) + pmax(pm - ses, 0) * (pm >= 0))
            ...
            if (length(vars) == 1) {
                ...
                if (mplot) {
                  ...
                  if (is.null(plottitle) || plottitle %in% c("NULL", "")) {
                    mtitle <- paste("Predicted means for \"", vars, "\" with ", bar_label, " (", slevel * 100,
                      "%) Bar", sep = "")
                  }

                  p1 <- ggplot(plotmt, aes(eval(parse(text = vars)), pm, group = 1)) + labs(title = paste(mtitle,
                    "\n", sep = ""), x = mxlab, y = mylab) + lims(x = c(bar_label, levels(plotmt[, vars])),
                    y = c(yMin - offSet, max(yMax + offSet, yMin + LSD_value + offSet))) + geom_point(colour = "red",
                    size = 2) + geom_errorbar(aes(ymax = up, ymin = yMin, x = bar_label), width = 0.15, linewidth = 0.8,
                    colour = "blue") + theme_bw(basesz)
                ...
                ## NB: I have changed this to an else statement because there shouldn't be any other way to
                ## get here. That is, length(vars) will not change from 1 to 2 between the block where
                ## length(vars)==1 is true and here, therefore the cases should be else if statements.
                ...
                if (barplot) {
                  ...
                  bp2 <- ggplot(plotmt, aes(eval(parse(text = fact1)), pm, group = eval(parse(text = fact2)),
                    ...
                    position = dodge) + labs(x = mxlab, y = mylab, title = mtitle) + ylim(c(min(0, (min(pm) -
                    max(ses)) * 1.1), max(0, (max(pm) + max(ses)) * 1.1))) + geom_errorbar(limits, position = dodge,
                    width = 0.25, colour = "blue") + theme_bw(basesz) + scale_fill_brewer() + theme(panel.border = element_blank(),
                    axis.line = element_line(), panel.grid = element_blank()) + theme(legend.position = "top") +
                ...
                ## NB: I have changed this to else if, because neither length(vars) or mplot should have
                ## changed, and I believe this is just the third choice
                if (newwd) {
                  dev.new()
                }
                ...
                if (is.null(plottitle) || plottitle %in% c("NULL", "")) {
                  mtitle <- paste("Predicted means for '", fact1, "' by '", fact2, "' for each '", fact3, "'\n with ",
                    paste(atvar, collapse = " "), " ", bar_label, " (", slevel * 100, "%) Bar\n", sep = "")
                }
                ...
                p3 <- ggplot(plotmt, aes(x = eval(parse(text = fact1)), y = pm, group = factor(eval(parse(text = fact2))),
                  col = factor(eval(parse(text = fact2))))) + labs(title = paste(mtitle, "\n", sep = ""), x = mxlab,
                  y = mylab) + lims(x = levels(plotmt[, fact1]), y = c(yMin - 0.5 * offSet, max(yMax + 0.5 *
                  offSet, yMin + LSD_value + 0.5 * offSet))) + geom_errorbar(aes(ymax = up, ymin = yMin, x = bar_label),
                  width = 0.15, linewidth = 0.8, colour = "blue") + geom_point(size = 2) + facet_grid(eval(parse(text = paste("~",
                  fact3, sep = "")))) + guides(group = guide_legend(fact2)) + guides(col = guide_legend(fact2)) +
                  theme_bw(basesz)

                if (lineplot) {
                  p3 <- p3 + geom_line(aes(linetype = eval(parse(text = fact2)), col = eval(parse(text = fact2))),
    ...
    if (!is.null(trans)) {
        ...
        if (plot && bkplot) {
            ...
            p <- ggplot(bkmt, aes(Mean, Trt)) + labs(title = mtitle, x = "", y = "") + xlim(xlimv) + geom_point(colour = "red") +
                geom_errorbarh(aes(xmax = UL, xmin = LL), height = 0.2, linewidth = 0.8, colour = "red") + scale_y_discrete(limits = rev(unique(bkmt$Trt))) +
                geom_point(aes(x = bky, y = Trtn), shape = 1, position = position_jitter(width = jitterv, height = jitterv),
                  colour = "blue", alpha = 0.6, data = newdata2) + theme_bw(basesz)
            ciPlot <- p
        ...
        # if (all(!identical(trans, function(x) x, ignore.environment=TRUE), !identical(trans, I,
        # ignore.environment=TRUE))) {
        if (any(meanTable$Mean != bkmt$Mean)) {
            meanTable <- cbind(meanTable, round(bkmt[, (ncol(bkmt) - 2):ncol(bkmt)], ndecimal))
            colnames(meanTable)[(ncol(meanTable) - 2):ncol(meanTable)] <- c("Bk_Mean", paste("Bk_LL(", (1 - slevel) *
                100, "%)", sep = ""), paste("Bk_UL(", (1 - slevel) * 100, "%)", sep = ""))
        }
        ...
        if (pairwise) {
            ...
            if ((is.null(atvar) || all(atvar %in% c("NULL", "")))) {
                ...
                if (letterCI) {
                  meanTable$LetterGrp <- ci_mcp(meanTable[, grepl("^LL", names(meanTable))], meanTable[, grepl("^UL",
                    names(meanTable))])
                }

                meanTable <- list(Table = meanTable, Note = paste("Letter-based representation of pairwise comparisons at significant level '",
                  slevel, "'", sep = ""))
                # attr(meanTable, 'Note') <- paste('Note: letter-based representation of pairwise
                # comparisons at significant level '', slevel, ''', sep='')
            } else {
                meanTable$LetterGrp <- unlist(pmlistLetter)
                meanTable <- list(Table = meanTable, Note = paste("Letter-based representation of pairwise comparisons at significant level '",
                  slevel, "' at each level of ", atvar, sep = ""))
            }
        }
    ...
    if (pairwise) {
        ...
        if (is.null(atvar) || all(atvar %in% c("NULL", ""))) {
            ...
            if (all(is.null(permlist) || all(permlist %in% c("NULL", "")), adj %in% c("none", "bonferroni"))) {
                outputlist <- list(`Predicted Means` = mean.table, `Standard Error of Means` = se.table, `Standard Error of Differences` = SED.out,
                  LSD = LSD, `Pairwise LSDs` = round(LSDm, ndecimal + 1), `Pairwise p-value` = round(t.p.valuem,
                    4), predictmeansPlot = predictmeansPlot, predictmeansBarPlot = predictmeansBarPlot, mean_table = meanTable,
                ...
                outputlist <- list(`Predicted Means` = mean.table, `Standard Error of Means` = se.table, `Standard Error of Differences` = SED.out,
                  LSD = LSD, `Pairwise p-value` = round(t.p.valuem, 4), predictmeansPlot = predictmeansPlot,
            ...
            ## DONGWEN: I think this could be replaced with outputlist[[6:(listlength + 5)]] <-
            ## pmListTab[[1:listLength]]
            ...
            if (is.null(permlist) || all(permlist %in% c("NULL", ""))) {
                names(outputlist) <- c("Predicted Means", "Standard Error of Means", "Standard Error of Differences",
                  "LSD", paste("Pairwise comparison p-value (adjusted by '", adj, "' method)", sep = ""), atvar.levels,
                  "mean_table", "predictmeansPlot", "predictmeansBarPlot", "p_valueMatrix")[1:length(outputlist)]
            } else {
                names(outputlist) <- c(c("Predicted Means", "Standard Error of Means", "Standard Error of Differences",
                  "Approximated LSD"), paste("Pairwise '", nsim, "' times permuted p-value (adjusted by '", adj,
                  "' method)", sep = ""), atvar.levels, "mean_table", "predictmeansPlot", "predictmeansBarPlot",
                  "p_valueMatrix")[1:length(outputlist)]
            }
        ...
        outputlist = list(`Predicted Means` = mean.table, `Standard Error of Means` = se.table, `Standard Error of Differences` = SED.out,
            LSD = LSD, predictmeansPlot = predictmeansPlot, predictmeansBarPlot = predictmeansBarPlot, mean_table = meanTable)
        class(outputlist) <- "pdmlist"
        return(outputlist)
    }
    # }
