...
#' predictout <- predictmeans(fm, "nitro:Variety",
#'   atvar = "Variety", adj = "BH",
...
predictmeans <- function(model,
  ...
  if (!is.null(plotord) && !any(c("NULL", "") %in% plotord)) {
    plot <- mplot <- TRUE
  ...
  if (nK == 1) {
    ...
    if (pairwise) {
      ...
      rownames(tvm) <- colnames(tvm) <- rownames(t.p.valuem) <- colnames(t.p.valuem) <- rownames(LSDm) <-
        colnames(LSDm) <- rnK
      t.v <- cm / dses
      ...
      if (is.null(atvar) || any(c("NULL", "") == atvar)) {
        ...
        if (is.null(permlist) || any(c("NULL", "") %in% permlist)) {
          ...
          attr(LSDm, "Note") <-
            paste("LSDs matrix has mean differences (row-col) above the diagonal, LSDs (adjusted by '",
                  adj, "' method) below the diagonal",
                  sep = "")
        } else {
          attr(t.p.valuem, "Note") <-
            paste("The matrix has t-value above the diagonal, and ", nsim, " times permutation p-value (adjusted by '",
    ...
    if (plot) {
      ...
      plotmt <- na.omit(mt)
      if (is.null(atvar) || any(c("NULL", "") == atvar)) {
        LSD_value <- ifelse(lsd_bar, LSD[3], SED.out[3])
      } else {
        LSD_value <- ifelse(lsd_bar, mean(attr(LSD, "For the Same Level of Factor")[1, atvar]),
          mean(attr(SED.out, "For the Same Level of Factor")[1, atvar])
        )
      }
      bar_label <- ifelse(lsd_bar, "Aveg.LSD", "Aveg.SED")
      ...
      if (length(vars) == 1) {
        if (mplot) {
          ...
          meanPlot <- mean_plot(plotmt, vars, y_var = "pm",
                                title = plottitle,
                                xlab = plotxlab,
                                ylab = response,
                                bar_value = LSD_value,
        ...
        if (barplot) {
          ...
          predictmeansBarPlot <- bar_plot(plotmt, vars, y_var = "pm",
                                          se_var = "ses",
                                          title = plottitle,
                                          xlab = plotxlab,
                                          ylab = response,
      ...
      if (length(vars) == 2) {
        ...
        if (mplot) {
          ...
          meanPlot <- mean_plot(plotmt, fact1, y_var = "pm", col_var = fact2,
                                title = plottitle,
                                xlab = plotxlab,
                                ylab = response,
                                bar_value = LSD_value,
        ...
        if (barplot) {
          ...
          predictmeansBarPlot <- bar_plot(plotmt, fact1, y_var = "pm", se_var = "ses", col_var = fact2,
                                          title = plottitle,
                                          xlab = plotxlab,
                                          ylab = response,
                                          basesz = basesz)
      ...
      if (all(length(vars) == 3)) {
        if (is.null(plotord) || any(c("NULL", "") %in% plotord)) {
          plotord <- 1:3
          if (!(is.null(atvar) || any(c("NULL", "") == atvar)) && length(atvar) == 1) {
            atvar_num <- which(vars %in% atvar)
            plotord <- c(atvar_num, setdiff(seq_along(vars), atvar_num))
          }
        }
        ...
        if (mplot) {
          ...
          meanPlot <- mean_plot(plotmt, fact1, y_var = "pm", col_var = fact2, panel_var = fact3,
                                title = plottitle,
                                xlab = plotxlab,
                                ylab = response,
                                bar_value = LSD_value,
  ...
  if (!is.null(trans)) {
    ...
    if (plot && bkplot) {
      ...
      p <- ggplot(bkmt, aes(Mean, Trt)) +
        ...
        geom_errorbarh(aes(xmax = UL, xmin = LL), height = 0.2, linewidth = 0.8, colour = "red") +
        scale_y_discrete(limits = rev(unique(bkmt$Trt))) +
        geom_point(aes(x = bky, y = Trtn),
          shape = 1, position = position_jitter(width = jitterv, height = jitterv),
          colour = "blue", alpha = 0.6, data = newdata2
        ) +
        theme_bw(basesz)
      ciPlot <- p
      if (prtplt) {
    ...
    # !identical(trans, I, ignore.environment=TRUE))) {
    ...
    if (is.null(atvar) || any(c("NULL", "") == atvar)) {
      ...
      if (is.null(permlist) || any(c("NULL", "") %in% permlist)) {
        names(outputlist) <- c(
          ...
          atvar.levels, "mean_table", "predictmeansPlot", "predictmeansBarPlot", "p_valueMatrix"
