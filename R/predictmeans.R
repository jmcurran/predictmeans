predictmeans <- function (model, modelterm, data=NULL, pairwise=FALSE, atvar=NULL, adj="none", Df=NULL, lsd_bar=TRUE,
                          ...
                          basesz=12L, prtnum=TRUE, prtplt=TRUE, newwd=FALSE, permlist=NULL, ncore=3L, ndecimal=4L) { # print
  options(scipen=6) 
    ...
    if (plot) {
      ...
      if (length(vars) == 1) {
        ...
        if (mplot) {
          ...
          mtitle <- plottitle
          if (is.null(plottitle) || plottitle %in% c("NULL", "")) {
            mtitle <- paste("Predicted means for \"", vars, "\" with ", bar_label, " (", pmlevel * 100, "%) Bar", sep="") 
          }
      ...
      if (length(vars) == 2) {
        ...
        if (mplot) {     
          ...
          if (is.null(plottitle) || plottitle %in% c("NULL", "")) {
            mtitle <- paste("Predicted means for \"", fact1, "\" by \"", fact2, "\" with ", 
                            paste(atvar, collapse=" "), " ", bar_label, " (", pmlevel * 100, "%) Bar", sep = "")
          }
        ...
        if (barplot) {
          ...
          dodge <- position_dodge(width=0.9)
          bp2 <- ggplot(plotmt, aes(eval(parse(text = fact1)), pm, 
      ...
      if (all(length(vars) == 3, mplot)) {
        ...
        if (is.null(plottitle) || plottitle %in% c("NULL", "")) {
          mtitle <- paste("Predicted means for '", fact1, "' by '", fact2, "' for each '",
                          fact3, "'\n with ", paste(atvar, collapse=" "), " ", bar_label, " (", 
                          pmlevel * 100, "%) Bar\n", sep = "")
        }
        mxlab <- plotxlab
        if (is.null(plotxlab) || plotxlab %in% c("NULL", "")) {
        ...
        p3 <- ggplot(plotmt, aes(eval(parse(text = fact1)), pm, 
