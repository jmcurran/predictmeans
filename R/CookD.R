...
CookD <- function(model, group = NULL, plot = TRUE, idn = 3, newwd = FALSE) {
  ...
  if (is.null(group) || group %in% c("NULL", "")) {
    ...
    LOOmp <- lapply(rn, function(x) {
      mymodelparm(update(model, data = mdf[rn != x, ]))
    })
  } else {
    rn <- unique(mdf[, group])
    LOOmp <- lapply(rn, function(x)  {
  ...
  LOObeta <- sapply(LOOmp, function(x) {
    x$coef         # Matrix with beta(-i)
  })
  rK <- t(LOObeta - beta0)
  ...
  if (plot) {
    ...
    plot(
      ...
      xlab = "Obs. number",
      col = "blue",
      ylim = c(0, max(CookD) + 0.005),
      main = "Cook's Distance",
      ylab = "Cook's distance",
      type = "h"
    )
    ## Plot annotation
    text(seq_along(CookD)[outD], CookD[outD], labid[outD], pos = 3)
