...
#' predcted data.}
#' @importFrom ggplot2 element_text geom_contour_filled geom_hline ggtitle
#' @importFrom Matrix .bdiag Matrix nearPD
#' @importFrom plotly plot_ly
#' @importFrom stats fitted lm model.response na.omit sigma
...
semipred <- function(semireg,
  ...
  if (!is.null(contr)) {
    trans <- NULL
    stopifnot("'modelterm' must be a single term" = length(unlist(strsplit(
      modelterm, "\\:"
    ))) == 1)
    ...
    if (is.character(contr)) {
      ...
      if (any(is.na(i))) {
        stop(paste(
          "The entries",
          paste0(contr[is.na(i)], sep = ", "),
          "do not match any level in ",
          modelterm
        ))
      } else {
        contr <- i
  ...
  if (!is.null(covariateV)) {
    if (length(covariate) > 1) {
      stopifnot(is.matrix(covariateV),
                dim(covariateV)[2] == length(covariate))
    ...
    for (i in covariate) {
      if (!is.null(attr(mod_df, paste(i, "mean", sep = "_")))) {
        if (!is.matrix(covariateV)) {
          ...
          covariateV[, i] <- covariateV[, i] - attr(mod_df,
                                                    paste(i, "mean", sep = "_"))
        }
      }
  ...
  pred_df <- KK$fctnames
  if (!(response %in% names(mod_df))) {
    # all.vars(str2expression(response0))]
  ...
  if (is.null(sm_term)) {
    ...
    if (length(sm_term) == 1) {
      ...
      sm_term <- intersect(names(vcov_indN), sm_term)
      sm_termInds <- seq_len(ncol(getME(semer, "X")))
      for (i in sm_term) {
        stmch_id <- match(i, names(vcov_indN))
        sm_termInds <- c(sm_termInds,
                         (vcov_indN[stmch_id - 1] + 1):vcov_indN[stmch_id])
      }
  ...
  for (i in sm_term) {
    ...
    if (!is.null(xcall_lst$type) &&
          xcall_lst$type == "smspline") {
      xcall_lst$pred <- TRUE
    }
  ...
  zstar <- qnorm(1 - level)
  LL <- y_hat - zstar * stderr
  UL <- y_hat + zstar * stderr
    ...
    if (!isLMM(semer)) {
      if (inherits(semer, "glmmTMB")) {
        ...
        weight_fun <- function(eta, mod) {
          ...
          family_fun$mu.eta(eta)^2 /
            family_fun$variance(family_fun$linkinv(eta))
        }
      } else {
      ...
      stderr_mu <- sig.epsHat * sqrt(diag(D_mx %*% sm_Cov %*% t(D_mx))) *
        weight_fun(y_hat, semer)
      y_hat <- invfun(y_hat)
      zstar <- qnorm(1 - level)
      LL <- y_hat - zstar * stderr_mu
      UL <- y_hat + zstar * stderr_mu
      if (family_n == "binomial") {
        UL <- ifelse(UL > 1, 1, UL)
        LL <- ifelse(LL < 0, 0, LL)
        if (is.factor(mod_df[[response]])) {
          mod_df[[response]] <- as.numeric(mod_df[[response]]) - 1
        } else if (!is.null(dim(mod_df[, response])) &&
                     ncol(mod_df[, response]) == 2) {
          mod_df[["Proportion"]] <- mod_df[, response][, 1] /
            rowSums(mod_df[, response])
          response <- "Proportion"
        }
      }
  ...
  if (!is.null(attr(mod_df, "numeric_var"))) {
    for (i in attr(mod_df, "numeric_var")) {
      if (!is.null(attr(mod_df, paste(i, "sd", sep = "_")))) {
        ...
        pred_df[[i]] <- pred_df[[i]] * attr(mod_df, paste(i, "sd", sep =
                                                            "_"))
      }

      if (!is.null(attr(mod_df, paste(i, "mean", sep = "_")))) {
        mod_df[[i]] <- mod_df[[i]] + attr(mod_df, paste(i, "mean", sep = "_"))
        pred_df[[i]] <- pred_df[[i]] + attr(mod_df, paste(i, "mean", sep =
                                                            "_"))
      }
    }
  }
  ...
  if (!is.null(attr(mod_df, paste(response, "sd", sep = "_")))) {
    ...
    pred_df$UL <- pred_df$UL * attr(mod_df, paste(response, "sd", sep =
                                                    "_"))
  }

  if (!is.null(attr(mod_df, paste(response, "mean", sep = "_")))) {
    pred_df$LL <- pred_df$LL + attr(mod_df, paste(response, "mean", sep = "_"))
    pred_df$UL <- pred_df$UL + attr(mod_df, paste(response, "mean", sep =
                                                    "_"))
  }
  ...
  if (setequal(modelterm, covariate)) {
    if (length(covariate) == 1) {
      plt <- ggplot(pred_df,
                    aes(x = eval(parse(text = covariate)), y
      ...
      if (point) {
        plt <- plt +
          geom_point(
            data = mod_df,
            aes(x = eval(parse(text = covariate)),
      ...
      if (threeD) {
        ...
        if (point) {
          plt <- plotly::add_trace(
            plt,
            x = mod_df[[covariate[1]]],
            y = mod_df[[covariate[2]]],
            z = mod_df[[response]],
            mode = "markers",
            type = "scatter3d",
            marker = list(
        ...
        plt <- ggplot(pred_df, aes(
          x = eval(parse(text = covariate[1])),
          y = eval(parse(text = covariate[2])),
          z = eval(parse(text = response))
        )) +
          geom_contour_filled() +
          labs(x = covariate[1], y = covariate[2], z = response)

        if (point) {
          plt <- plt + geom_point(data = mod_df,
                                  col = "red",
                                  size = 1.5)
        }
    ...
    if (length(vars) == 1) {
      if (length(covariate) == 1) {
        if (is.null(contr)) {
          plt <- ggplot(pred_df, aes(
            x = eval(parse(text = covariate)),
            y = eval(parse(text = response)),
            col = eval(parse(text = modelterm))
          )) +
            geom_line(aes(
              x = eval(parse(text = covariate)),
              y = eval(parse(text = response)),
          ...
          if (ci)
            plt <- plt +
              geom_ribbon(aes(
                ymin = LL,
                ymax = UL,
                fill = eval(parse(text = modelterm))
              ),
              alpha = 0.3,
              stat = "identity") +
              labs(fill = modelterm)
          ...
          grp_fct_levl <- levels(grp_fct)
          plt_df_lst <- lapply(split(pred_df, grp_fct), function(x) {
            x[, !names(x) %in% modelterm]
          })
          plt_df_contr <- plt_df_lst[[grp_fct_levl[contr[1]]]] -
            plt_df_lst[[grp_fct_levl[contr[2]]]]
          diff_covarn <- setdiff(names(plt_df_contr),
                                 c("SE", response, "LL", "UL"))
          plt_df_contr[, diff_covarn] <-
            plt_df_lst[[grp_fct_levl[contr[1]]]][, diff_covarn]

          D_mx_lst <-
            lapply(split(data.frame(as.matrix(D_mx)), grp_fct), as.matrix)
          D_contr <-
            D_mx_lst[[grp_fct_levl[contr[1]]]] -
            D_mx_lst[[grp_fct_levl[contr[2]]]]

          if (!isLMM(semer)) {
            stderr_contr <- sig.epsHat *
              sqrt(diag(D_contr %*% sm_Cov %*% t(D_contr))) *
              weight_fun(plt_df_contr[[response]], semer)
          } else {
            stderr_contr <-
              sig.epsHat * sqrt(diag(D_contr %*% sm_Cov %*% t(D_contr)))
          }

          if (!is.null(attr(mod_df, paste(response, "sd", sep = "_")))) {
            zstar <- qnorm(1 - level)
            plt_df_contr$LL <- plt_df_contr[[response]] - zstar * stderr_contr *
              attr(mod_df, paste(response, "sd", sep = "_"))
            plt_df_contr$UL <- plt_df_contr[[response]] + zstar *
              stderr_contr * attr(mod_df, paste(response, "sd", sep = "_"))
          } else {
            zstar <- qnorm(1 - level)
            plt_df_contr$LL <- plt_df_contr[[response]] - zstar * stderr_contr
            plt_df_contr$UL <- plt_df_contr[[response]] + zstar *
          ...
          plt <- ggplot(plt_df_contr, aes(x = eval(parse(text = covariate)), y =
                                            eval(parse(text = response)))) +
            ...
            ggtitle(
              paste(
                ...
                modelterm,
                "level",
                grp_fct_levl[contr[1]],
                "vs level",
                grp_fct_levl[contr[2]],
                "with",
                (1 - level) * 100,
                "% CI"
              )
            ...
            labs(x = covariate,
                 y = paste("The difference of ", response, "\n", sep = "")) +
            theme_bw() +
            theme(plot.title = element_text(hjust = 0.5))

          if (response == "response_y") {
            plt <- plt + labs(y = paste("The difference of ",
                                        response0, "\n",
                                        sep = ""))
          }
        ...
        if (is.null(contr)) {
          ...
          if (threeD) {
            ...
            if (point) {
              ...
              for (i in names(mod_df_lst)) {
                plt[[i]] <- plotly::add_trace(
                  plt[[i]],
                  x = mod_df_lst[[i]][[covariate[1]]],
                  y = mod_df_lst[[i]][[covariate[2]]],
                  z = mod_df_lst[[i]][[response]],
                  mode = "markers",
                  type = "scatter3d",
                  marker = list(
                    size = 2.5,
          ...
          D_mx_lst <- lapply(split(data.frame(as.matrix(D_mx)), grp_fct),
                             as.matrix)
          D_contr <- D_mx_lst[[grp_fct_levl[contr[1]]]] -
            D_mx_lst[[grp_fct_levl[contr[2]]]]
          ...
          plt <- ggplot(plt_df_contrN, aes(
            ...
            ggtitle(
              paste(
                ...
                "vs level",
                grp_fct_levl[contr[2]],
                "with Points at the Area out of",
                (1 - level) * 100,
                "% CI"
              )
            ) +
          ...
          if (response == "response_y") {
            plt <- plt +
              labs(y = paste("The difference of ", response0, "\n", sep = ""))
          }
  ...
  if (prt) {
    ...
    if (length(covariate) == 2 &&
          threeD && !setequal(modelterm, covariate)) {
