semipred <- function(semireg, modelterm=NULL, covariate, sm_term=NULL, contr=NULL,
  ...
  if (setequal(modelterm, covariate)) {
    if (length(covariate) == 1) {
      ...
      if (point) {
        plt <- plt + 
          geom_point(data=mod_df, aes(x=eval(parse(text = covariate)), y=eval(parse(text = response))), 
      ...
      if (threeD) {
        ...
        plt <- ggplot(pred_df, aes(x=eval(parse(text = covariate[1])), 
    ...
    if (length(vars) == 1) {
      if (length(covariate) == 1) {
        if (is.null(contr)) {
          plt <- ggplot(pred_df, aes(x=eval(parse(text = covariate)), 
                                     y=eval(parse(text = response)), 
                                     col=eval(parse(text = modelterm))))+
            geom_line(aes(x=eval(parse(text = covariate)), y=eval(parse(text = response)), 
                          col=eval(parse(text = modelterm))), linewidth=1) +
            labs(x=covariate, y=response, col=modelterm) +
            theme_bw()	
          if (ci) {
            plt <- plt + 
              geom_ribbon(aes(ymin = LL, ymax = UL, fill=eval(parse(text = modelterm))), 
                          alpha = 0.3, stat="identity")+
              labs(fill=modelterm)
          }
          if (point) {
            plt <- plt + 
              geom_point(data=mod_df, aes(x=eval(parse(text = covariate)), 
                                          y=eval(parse(text = response)), 
                                          col=eval(parse(text = modelterm))), 
        ...
        if (is.null(contr)) {
          ...
          plt <- ggplot(plt_df_contrN, aes(x=eval(parse(text = covariate[1])), 
                                           y=eval(parse(text = covariate[2])), 
                                           z=eval(parse(text = response)))) + 
            geom_contour_filled() +
      ...
      plt <- ggplot(pred_df, aes(x=eval(parse(text = covariate)), y=eval(parse(text = response)), 
        ...
        geom_line(aes(x=eval(parse(text = covariate)), y=eval(parse(text = response)), 
                      col=eval(parse(text = fact1))), linewidth=1) +
      ...
      if (ci) {
        plt <- plt +
          geom_ribbon(data=pred_df, aes(ymin = LL, ymax = UL, fill=eval(parse(text = fact1))), 
                      alpha = 0.3, stat="identity") +
          labs(fill=fact1)
      }
      if (point) {
