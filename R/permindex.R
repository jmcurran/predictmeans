...
permindex <- function(data,
  ...
  if (all(is.null(block), is.null(group))) {
    ...
    ## split 1:n seq into nB blocks
    Bsplit <- split(seq_len(n), as.character(data[, block]))

    if (!missing(seed)) {
      set.seed(seed + 1)
    }

    permIndex <- replicate(nsim, unlist(lapply(Bsplit, function(x) {
      x[sample.int(length(x))]
    }),
    recursive = FALSE,
    use.names = FALSE))
  } else if (all(is.null(block), !is.null(group))) {
    ## data=data; group="group"; nsim=5
    if (!missing(seed)) {
      set.seed(seed + 2)
    }

    permIndex <- replicate(nsim, {
      ## split 1:n seq into separate groups
      Gsplit <- split(seq_len(n), as.character(data[, group]))
      nGsplit <- lapply(Gsplit, function(x) {
        ## within each group permute the whole seq
        x[sample.int(length(x))]
      })
      unlist(nGsplit[sample.int(length(nGsplit))], recursive = FALSE,
             use.names = FALSE)
    }) ## permute the order of the whole groups and repeat process nsim times
  } else {
    ind <- seq_len(n)
    names(ind) <- as.character(data[, group])
    ## split 1:n seq into blocks
    Bsplit <- split(ind, as.character(data[, block]))

    if (!missing(seed)) {
      set.seed(seed + 3)
    }

    permIndex <- replicate(nsim, unlist(
      lapply(Bsplit, function(x) {
        ## within each block
        ## split each block seq into groups
        Gsplit <- split(x, names(x))
        nGsplit <- lapply(Gsplit, function(x) {
          ## within each group permute the whole seq
          x[sample.int(length(x))]
        })
        unlist(nGsplit[sample.int(length(nGsplit))], recursive = FALSE,
               use.names = FALSE) ## permute the order of groups
      }),
      recursive = FALSE,
      use.names = FALSE
    ))
  ...
  colnIndex <- apply(permIndex, 2, function(x) {
    paste(x, collapse = "")
  })
