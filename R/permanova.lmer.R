...
permanova.lmer <- function(model,
  ...
  if (inherits(try(refit(model, getME(model, "y")), TRUE)
               , "try-error")) {
  ...
  model.0 <- update(model, as.formula(paste(
    ".~. -", paste(termlabel0, collapse = "-")
  )))
  ...
  for (vars in termlabel1) {
    if (type == "3") {
      ...
      Perm.p[vars] <- permlmer(model.b, model, nperm, ncore,
                               plot = FALSE, ...)$`Perm-p`[2]
      termlabel2 <- termlabel1
    } else if (type == "2") {
      varsn <- unlist(strsplit(vars, "\\:"))
      model.1 <- update(model.0, as.formula(paste(
        ".~.+",
        paste(termlabel0[attr(terms(model), "order") %in% (seq_along(varsn))],
              collapse = "+")
      )))
      model.2 <- update(model.1, as.formula(paste(".~. -", vars)))
      Perm.p[vars] <- permlmer(model.2, model.1, nperm, ncore,
                               plot = FALSE, ...)$`Perm-p`[2]
    } else if (type == "1") {
      model.1 <- update(model.0, as.formula(paste(".~. +", vars)))
      Perm.p[vars] <- permlmer(model.0, model.1, nperm, ncore,
                               plot = FALSE, ...)$`Perm-p`[2]
      model.0 <- model.1
    } else {
      NULL
