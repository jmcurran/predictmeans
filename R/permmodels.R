...
permmodels <- function(model,
  ...
  if (any(inherits(model, "lm"), inherits(model, "aov"), inherits(model, "glm"))) {
    ...
    if (!is.null(fo)) {
      ...
      if (.Platform$OS.type == "windows") {
        ...
        clusterExport(cl, c(
          "mymodelparm", "mymodelparm.default", "model",
  ...
  if (any(inherits(model, "gls"), inherits(model, "lme"))) {
    ...
    if (!is.null(fo)) {
      ...
      if (.Platform$OS.type == "windows") {
        ...
        clusterExport(cl, c(
          "mymodelparm", "mymodelparm.lme", "mymodelparm.gls",
  ...
  if (any(inherits(model, "lmerMod"), inherits(model, "glmerMod"))) {
    ...
    if (!is.null(fo)) {
      ...
      if (.Platform$OS.type == "windows") {
        ...
        clusterExport(cl, c(
          "mymodelparm", "mymodelparm.lmerMod", "mymodelparm.default",
          "model", "type", "test.statistic"
        ), envir = environment())
        permmod <- parLapplyLB(cl, permy, function(x) {
          rfitmodel <- try(refit(model, x), TRUE)
          if (class(rfitmodel)[1] %in% c("lmerMod", "glmerMod")) {
  ...
  if (!(inherits(model, "aov"))) {
    ...
    # Tper.p <- (sum(round(sapply(permmod, function(x) {mp <- x[[1]]; abs(mp$coef/sqrt(base::diag(mp$vcov)))}), 6) >=
    # round(abs(model_tTable[, Tvalue]),6))+!exact)/(nperm+!exact)
    # }else{
    # Tper.p <- (rowSums(round(sapply(permmod, function(x) {mp <- x[[1]];
    # abs(mp$coef/sqrt(base::diag(mp$vcov)))}), 6) >= round(abs(model_tTable[, Tvalue]), 6))+!exact)/(nperm+!exact)
    # }

    # model_tTable <- coef(summary(model))
    # Tvalue <- colnames(model_tTable)[grep("value", colnames(model_tTable))][1]
    # if (nrow(model_tTable) == 1) {
    #   Tper.p <- (sum(round(sapply(permmod, function(x) abs(x[[3]][, Tvalue])), 6) >=
    # round(abs(model_tTable[, Tvalue]), 6)) + !exact) / (nperm + !exact)
    # } else {
    #   Tper.p <- (rowSums(round(sapply(permmod, function(x) abs(x[[3]][, Tvalue])), 6) >=
    # round(abs(model_tTable[, Tvalue]), 6)) + !exact) / (nperm + !exact)
    # }
    ...
    # Extract absolute test statistics from permutations
    perm_stats <- sapply(permmod, function(x) abs(x[[3]][, Tvalue]))
    obs_stats <- abs(model_tTable[, Tvalue])

    # Round once for both observed and permuted stats
    perm_stats <- round(perm_stats, 6)
    obs_stats  <- round(obs_stats, 6)

    # Compute p-values based on number of rows
    Tper.p <- if (nrow(model_tTable) == 1) {
      (sum(perm_stats >= obs_stats) + !exact) / (nperm + !exact)
    } else {
      (rowSums(perm_stats >= obs_stats) + !exact) / (nperm + !exact)
    }
