...
permmodels <- function(model, nperm = 999, type = c("I", "II", "III", 1, 2, 3),
  ...
  if (any(inherits(model, "lmerMod"), inherits(model, "glmerMod"))) {
    ...
    theta <- getME(model, "theta") # nolint: object_usage_linter Doesn't appear to be used
    fixef <- fixef(model)
  ...
  if (!(inherits(model, "aov"))) {
    ...
    # }else{
    # Tper.p <- (rowSums(round(sapply(permmod, function(x) {mp <- x[[1]]; abs(mp$coef/sqrt(base::diag(mp$vcov)))}), 6)
    # >= round(abs(model_tTable[, Tvalue]), 6))+!exact)/(nperm+!exact)
    # }
