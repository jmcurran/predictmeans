...
#' zosuMat_by <- smZ(x, intKnots = knots, by=f, group=TRUE)
#' str(zosuMat_by)
#'
#' @export smZ
smZ <- function(x,
  ...
  if (type == "ZOSull") {
    if (is.factor(by)) {
      if (is.null(intKnots)) {
        ...
        probs <- seq(0, 1, length = k + 2)[-c(1, k + 2)]
    ...
    if (is.factor(by)) {
      ...
      if (pred)
        ZZ <- lmeSplines::approx.Z(Z = range.x,
                                   oldtimes = intKnots,
                                   newtimes = x)
      ...
      if (pred)
        Z <- lmeSplines::approx.Z(Z = range.x,
                                  oldtimes = intKnots,
                                  newtimes = x)
      if (nas) {
        nmat <- matrix(NA, length(nax), ncol(Z))
