...
#'                + stunted + visit2 + visit3 + visit4  + visit5
#'                + visit6+(1|idnum),
#'                smoothZ=list(
#'                  grp=smZ(age)
#'                ),
...
semireg <- function(formula,
  ...
  if (any(ncenter, nscale)) {
    for (i in numeric_n) {
      ...
      if (!is.matrix(data_i)) {
        ...
        if (ncenter) {
          attr(data, paste(i, "mean", sep = "_")) <- attr(scaled_i,
                                                          "scaled:center")
        }

        if (nscale) {
          attr(data, paste(i, "sd", sep = "_")) <- attr(scaled_i,
                                                        "scaled:scale")
        }
      }
    }
  ...
  # for (i in setdiff(names(x_N_attr), c("dim", "class")))
  #   attr(x_N,i) <- x_N_attr[[i]]
  ...
  for (i in Zt_name) {
    ...
    lmerc$formula <- paste(paste(format(lmerc$formula), collapse = ""),
                           "+ (1|", i, ")")
  }
  ...
  if (predict_info) {
    ...
    u_ind <- sapply(getME(semer, "Ztlist"), function(x) {
      x@Dim[1]
    })
    ...
    knots_lst <- range_lst <- type_lst <- cov_lst <- u_lst <-
      fullCovMat <- vcov_ind <- vcov_indN <- df <- NULL
  }
