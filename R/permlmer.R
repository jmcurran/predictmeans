...
permlmer <- function(lmer0, lmer1, nperm = 999, ncore = 3L, plot = FALSE, seed) {
  ...
  # if(!setequal(intersect(theta0name, theta1name), theta0name) || !setequal(intersect(fixef0name, fixef1name),
  # fixef0name)) stop(paste("The model", lmernames[1], "must be nested within the model", lmernames[2]))
  ...
  if (.Platform$OS.type == "windows") {
    ...
    lrtest2 <- mclapply(permy, function(x) {
      LRT <- try(2 * (logLik(suppressMessages(refit(lmer1, x)), REML = ref) -
                        logLik(suppressMessages(refit(lmer0, x)), REML = ref)), TRUE)
      LRT <- ifelse(is.numeric(LRT), LRT, NA)
    }, mc.cores = ncore)
  }
  ...
  if (plot) {
    # dev.new()
    plot(density(c(lrtest1, lrtest), kernel = "epanechnikov"),
         col = "blue", lwd = 2, xlab = "", main = "LR Test's density kernels")
