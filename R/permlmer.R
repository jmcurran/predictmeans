...
permlmer <- function(lmer0,
  ...
  for (i in term0name) {
    for (j in term1name) {
      if (length(setdiff(unlist(strsplit(i, "\\:")),
                         unlist(strsplit(j, "\\:")))) == 0) {
        term0in1[i] <- TRUE
        break
      }
  ...
  ## if(!setequal(intersect(theta0name, theta1name), theta0name) ||
  ##   !setequal(intersect(fixef0name, fixef1name), fixef0name)) {
  ##   stop(paste("The model", lmernames[1], "must be nested within the model",
  ##              lmernames[2]))
  ## }
  ...
  V1n <- getME(lmer1, "Z") %*% Lambda1nc %*% getME(lmer1, "Zt") +
    diag(dim(getME(lmer1, "Z"))[1])
  Ut <- t(chol(V1n))
  wt <- solve(Ut)
  ...
  if (ref) {
    ...
    permy <- as.data.frame(xbeta +
                             replicate(nperm,
                                       as.vector(Ut %*% sample(wterrors))))
  } else {
    if (!missing(seed)) {
      set.seed(seed)
    }
  ...
  if (.Platform$OS.type == "windows") {
    ...
    lrtest2 <- parLapplyLB(cl, permy, function(x) {
      LRT <- try(2 * (logLik(refit(lmer1, x), REML = ref) -
                        logLik(refit(lmer0, x), REML = ref)), TRUE)
      LRT <- ifelse(is.numeric(LRT), LRT, NA)
    })
    stopCluster(cl)
  } else {
    lrtest2 <- mclapply(permy, function(x) {
      LRT <- try(2 * (logLik(suppressMessages(refit(lmer1, x)), REML = ref) -
                        logLik(suppressMessages(refit(lmer0, x)), REML = ref)),
                 TRUE)
      LRT <- ifelse(is.numeric(LRT), LRT, NA)
    }, mc.cores = ncore)
  }
  ...
  ## I have used double quotes to keep lintr quiet. Not sure I like this.
  ## TODO: Change?
  ...
  if (plot) {
    dev.new()
    plot(
      density(c(lrtest1, lrtest), kernel = "epanechnikov"),
      col = "blue",
      lwd = 2,
      xlab = "",
      main = "LR Test's density kernels"
    )
