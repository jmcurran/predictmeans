...
ci_plot <- function(plot_mt, mod_df = NULL, resp_name = NULL,
                    jitterv = 0.2, basesz = 12) {
  ...
  ciPlot <- ggplot(plot_mt, aes(Mean_v, Treat)) +
    ...
    geom_errorbarh(aes(xmax = UL, xmin = LL), height = 0.2, linewidth = 0.8,
                   colour = "red") +
    scale_y_discrete(limits = rev(plot_mt[["Treat"]])) +
    theme_bw(basesz)
  if (is.data.frame(mod_df)) {
    ciPlot <- ciPlot +
      geom_point(aes(x = .data[[resp_name]], y = .data[["Treat"]]),
        shape = 1,
