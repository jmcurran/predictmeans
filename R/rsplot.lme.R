...
rsplot.lme <- function(model,
  ...
  # cls <- sapply(all.vars(formula(model, fixed.only=TRUE)),
  # function(x) class(model.frame(model)[[x]])[1])
  # } else { cls <- attr(tm,"dataClasses")
  # }
  #  yname <- names(cls)[1]
  ...
  if (inherits(model, "glmerMod") || inherits(model, "glmmTMB")) {
    ...
    if (inherits(model, "glmmTMB")) {
      ...
      RandomEffects <- getME(model, "Z") %*%
        unlist(lme4::ranef(model)[["cond"]])
    } else {
    ...
    tmp <- qqnorm(
      residv,
      col = "blue",
      main = "Normal Plot for Residuals",
      xlab = "Standardized residuals",
      ylab = "Quantiles of Standard Normal"
    )
    qqline(residv)
    lab.df <- data.frame(x = tmp$x,
                         y = tmp$y,
                         lab = rownames(mf))
    lab.df <- lab.df[order(lab.df$x), ]
    with(lab.df[c(1:3, (dim(lab.df)[1] - 2):dim(lab.df)[1]), ],
         text(x, y, labels = lab))
  }
  ...
  if (unique(group == "none")) {
    ...
    if (unique(group %in% names(mf))) {
      ...
      if (!length(colf) == nrow(mf)) {
        stop(paste0("The length of 'group' must be the same as the other ",
                    "predicted variable!"))
      }
    }
  ...
  if (inherits(model, "glmerMod") &&
        length(unique(model@resp$y)) == 2) {
    ## plot logistic curve, mean change, T/F
    observed <- model@resp$y
    sf <- sort(fittedv, index = TRUE)   # sort the fitted values
    plot(
      sf$x,
      ylim = c(0, 1),
      type = "s",
    ...
    points(
      ...
      pch = "|",
      cex = 1,
      col = ifelse(as.integer(observed[sf$ix]), "green4", "maroon1")
    )
  } else {
    plot(
      fittedv ~ obsv,
