...
rsplot.lme <- function(model, group = "none", level = 1, slope = FALSE, id = FALSE, ask = FALSE) {
  ...
  #  tm <- terms(model)
  #  if (class(model)[1]%in%c("lmerMod", "glmerMod")) cls <-
  # sapply(all.vars(formula(model, fixed.only=TRUE)),
  # function(x) class(model.frame(model)[[x]])[1]) else cls <- attr(tm,"dataClasses")
  #  yname <- names(cls)[1]
  ...
  if (inherits(model, "lme")) {
    ...
    if (inherits_any(model, c("lmerMod", "merModLmerTest", "glmerMod", "glmmTMB"))) {
      if (slope) {
  ...
  if (inherits(model, "glmerMod") || inherits(model, "glmmTMB")) {
    ...
    tmp <- qqnorm(residv,
      col = "blue", main = "Normal Plot for Residuals",
  ...
  if (unique(group == "none")) {
    plot(residv ~ fittedv,
      col = "blue", main = "Residuals vs Fitted",
    ...
    plot(residv ~ fittedv,
      col = colf, main = "Residuals vs Fitted",
      ylab = "Standardized residuals", xlab = "Fitted values"
    )
    loess.fit <- loess.smooth(fittedv, residv)
    lines(loess.fit$x, loess.fit$y, col = "red", lty = 1, lwd = 0.4)
  ...
  if (inherits(model, "glmerMod") && length(unique(model@resp$y)) == 2) {
    ...
    plot(sf$x,
      ylim = c(0, 1), type = "s", col = "blue", lwd = 2,
    ...
    # show T/F
    points(seq_along(observed), observed[sf$ix],
