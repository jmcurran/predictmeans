...
R2_glmm <- function(model, over_disp = FALSE) {
  stopifnot(inherits_any(model, c("lmerMod", "glmerMod", "glmmTMB")))
  ...
  cod_v <- c(Total = Omega_beta_u_percent, Fixed = Omega_beta_percent,
             Random = Omega_u_percent, unlist(Omega_u_list) * 100)
