...
R2_glmm <- function(model, over_disp = FALSE) {
  ...
  if (inherits(model, "glmmTMB")) {
    ...
    if (inherits(tmbf, "try-error")) {
      tmb_call$data <- base::merge(eval(tmb_call$data), model$frame,
                                   sort = FALSE)
      tmbf <- eval(tmb_call)
    }
  ...
  if (family_n != "gaussian") {
    if (family_n == "binomial" && all(yobs %in% c(0, 1))) {
      ...
      if (inherits(model, "glmmTMB")) {
        ...
        prior_weight <- as.vector(weight_fun(y_hat, model)) *
          tmbf$data.tmb$weights
  ...
  theta_V <- tr(V %*% P) / (n - 1)
  theta_Xbeta <- t(beta) %*% t(X) %*% P %*% X %*%
    (beta / (n - 1)) - tr(t(X) %*% P %*% X %*% covB) / (n - 1)
  if (over_disp) {
    theta_Zu <- tr(P %*% Z %*% G %*%
                     diag(c(rep(0, n), rep(1, nrow(G) - n))) %*% Zt) / (n - 1)
  } else {
  ...
  if (!over_disp) {
    if (inherits(model, "glmmTMB")) {
      ...
      u_ind <- c(ini = 0, sapply(getME(model, "Ztlist"), function(x) {
        x@Dim[1]
      }))
      u_ind <- cumsum(u_ind)
    ...
    for (i in seq_along(Omega_u_list)) {
      contInds <- (u_ind[i] + 1):(u_ind[i + 1])
      theta_Zu_i <- tr(P %*% Z[, contInds] %*% G[contInds, contInds] %*%
                         Zt[contInds, ]) / (n - 1)
  ...
  cod_v <- c(
    Total = Omega_beta_u_percent,
    Fixed = Omega_beta_percent,
    Random = Omega_u_percent,
    unlist(Omega_u_list) * 100
  )
