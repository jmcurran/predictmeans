...
#'   covariatemeans(fm, "Variety", covariate = "nitro",
#'                  covariateV = seq(0, 0.6, 0.1))$pltdf
#'
...
covariatemeans <- function(model,
  ...
  if (!setequal(modelterm, covariate) && as.is) {
    ...
    pltdf <- merge(
      merge(pltdf, min_df, by = vars, sort = FALSE),
      max_df,
      by = vars,
      sort = FALSE
    )
    pltdf <- pltdf[pltdf[[covariate]] >= pltdf[["min_value"]] &
                     pltdf[[covariate]] <= pltdf[["max_value"]], ]
  ...
  if (setequal(modelterm, covariate)) {
    ...
    pltdf$factors <- factor(do.call("paste", c(pltdf[, vars, drop = FALSE],
                                               sep = ":")))
  }
  ...
  if (is.null(mtitle) || mtitle %in% c("NULL", "")) {
    mtitle <- paste("Fitted and observed relationship with",
                    (1 - level) * 100,
                    "% CI")
  }
  ...
  if (is.null(trans)) {
    ...
    if (length(setdiff(response, names(mdf))) == 0) {
      ...
      if (inherits(model, "glm") ||
        ...
        if (any(
          identical(trans, function(x) {
            x
          }, ignore.environment = TRUE),
          identical(trans, I, ignore.environment = TRUE)
        )) {
          if (inherits(model, "glm")) {
            mdf$bky <- ifelse(mdf$bky >= 1 |
                                mdf$bky <= 0,
                              NA,
                              model$family$linkfun(mdf$bky))
          }
          if (inherits(model, "glmerMod")) {
            mdf$bky <- ifelse(
              mdf$bky >= 1 |
                mdf$bky <= 0,
              NA,
              slot(model, "resp")$family$linkfun(mdf$bky)
            )
          }
          if (inherits(model, "glmmTMB")) {
            mdf$bky <- ifelse(mdf$bky >= 1 |
                                mdf$bky <= 0,
                              NA,
                              model$modelInfo$family$linkfun(mdf$bky))
          }
          response <- "Response"
        }
      } else {
  ...
  pltdf[, c("Mean", "LL", "UL")] <- lapply(pltdf[, c("Mean", "LL", "UL")],
                                           as.numeric)
  ...
  if (!trellis) {
    ...
    if (setequal(modelterm, covariate))  {
      ...
      plt <- plt + guides(col = guide_legend(modelterm),
                          fill = guide_legend(modelterm))
    }
    print(plt)
  } else {
    if (length(vars) == 1) {
    ...
    if (length(vars) == 2) {
      ...
      plt <- ggplot(pltdf, aes(xvar, Mean, colour = factor(eval(
        parse(text = fact1)
      )))) +
        labs(
          title = paste(mtitle, "\n", sep = ""),
        ...
        geom_line(linewidth = 0.5) +
        facet_grid(eval(parse(text = paste(
          "~", fact2, sep = ""
        )))) +
        theme_bw()
      if (ci) {
        plt <- plt + geom_ribbon(
          aes(
            ymin = LL,
            ymax = UL,
            fill = factor(eval(parse(text = fact1)))
          ),
          alpha = 0.2,
          data = pltdf,
          stat = "identity"
