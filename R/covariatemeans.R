...
#' @export
covariatemeans <- function(model, modelterm = NULL, covariate, as.is = FALSE, covariateV = NULL, data = NULL,
                           level = 0.05, Df = NULL, trans = NULL, transOff = 0, responsen = NULL, trellis = TRUE,
  ...
  if (is.null(trans)) {
    ...
    if (length(setdiff(response, names(mdf))) == 0) { ## Transformed y before modelling
      if (inherits(model, "glm") || inherits(model, "glmerMod") || inherits(model, "glmmTMB")) {
        ...
        if (any(identical(trans, function(x) x, ignore.environment = TRUE),
  ...
  if (!trellis) {
    ...
    if (setequal(modelterm, covariate)) {
      ...
      if (point) {
        plt <- plt + geom_point(aes(x = xvar, y = bky),
                                position = position_jitter(width = jitterv, height = jitterv), data = mdf)
      }
    } else {
      plt <- ggplot(pltdf, aes(xvar, Mean, colour = factors)) +
        ...
        theme_bw()
      if (ci) {
        plt <- plt + geom_ribbon(aes(ymin = LL, ymax = UL, fill = factors),
                                 alpha = 0.2, data = pltdf, stat = "identity")
      }
      if (point) {
        plt <- plt + geom_point(aes(x = xvar, y = bky), position = position_jitter(width = jitterv, height = jitterv),
                                data = mdf)
      }
    ...
    if (length(vars) == 2) {
      ...
      if (ci) {
        plt <- plt +
          geom_ribbon(aes(ymin = LL, ymax = UL, fill = factor(.data[[fact1]])),
            alpha = 0.2, data = pltdf, stat = "identity"
          )
      }
      if (point) {
        plt <- plt +
          geom_point(aes(x = xvar, y = bky),
            position = position_jitter(width = jitterv, height = jitterv), data = mdf
          )
      }
