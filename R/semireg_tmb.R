...
#' families or \code{\link{family_glmmTMB}} for details of
...
#' each family, see \code{\link{sigma}}. The dispersion model uses a log link.
...
#' See \code{\link[TMB]{MakeADFun}} for more details.
...
#' \code{doFit = FALSE}.}
#'
#' @importFrom glmmTMB glmmTMBControl fitTMB
...
#' library(ggplot2)
#' sp_out3$plt +
#'   geom_rug(data = subset(indonRespir, respirInfec==0),
#'    sides = "b", col="deeppink") +
#'   geom_rug(data = subset(indonRespir, respirInfec==1),
#'    sides = "t", col="deeppink")+
#'   ylim(0, 0.2)
#' }
#' @export semireg_tmb
semireg_tmb <- function(formula,
  ...
  data <- droplevels(as.data.frame(na.omit(data[, intersect(all.vars(mc),
                                                            names(data))])))
  ...
  stopifnot(is.list(smoothZt),
            # check the smoothZt argument
            length(Zt_name) == length(smoothZt),
            all(sapply(smoothZt, is, class2 = "sparseMatrix")))

  ## adding the constructed variables to the semer frame avoiding name
  ## duplication
  for (i in Zt_name) {
    data[[i]] <- factor(rep(seq_len(nrow(smoothZt[[i]])),
                            length = ncol(smoothZt[[i]])))
    glmmTMBcall$formula <- paste(paste(format(glmmTMBcall$formula),
                                       collapse = ""), "+ (1|", i, ")")
  }
  ...
  if (predict_info) {
    ...
    knots_lst <- range_lst <- type_lst <- cov_lst <-
      u_lst <- fullCovMat <- vcov_ind <- vcov_indN <- df <- NULL
  }
  ans <- list(
    semer = semer,
    data = data,
    fomul_vars = fomul_vars,
    sm_vars = sm_vars,
    smoothZ_call = mc$smoothZ,
    knots_lst = knots_lst,
    range_lst = range_lst,
    type_lst = type_lst,
    cov_lst = cov_lst,
