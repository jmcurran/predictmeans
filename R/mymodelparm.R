...
#' @export
#' @keywords internal
mymodelparm.default <- function(model,
                                coef. = coef,
                                vcov. = vcov,
                                df = NULL,
                                ...) {
  ...
  if (is.null(df)) {
    ...
    if (inherits(model, "lmerMod")) {
      ...
      df <- sapply(L_list, function(L) {
        mean(df_term(model, ctrmatrix = L))
      })
    }
  ...
  ### just in case...
  if (length(beta) != ncol(sigma) ||
        nrow(sigma) != sum(estimable)) {
...
#' @export
#' @keywords internal
mymodelparm.aovlist <- function(model,
                                coef. = coef,
                                vcov. = vcov,
                                df = NULL,
                                ...) {
  stop("This function does not support objects of class ",
       sQuote("aovlist"))
}

#' @export
#' @keywords internal
mymodelparm.lme <- function(model,
                            coef. = nlme::fixef,
                            vcov. = vcov,
                            df = NULL,
                            ...) {
  mymodelparm.default(model,
                      coef. = coef.,
                      vcov. = vcov.,
                      df = df,
                      ...)
...
#' @export
#' @keywords internal
mymodelparm.lmerMod <- function(model,
                                coef. = lme4::fixef,
                                vcov. = vcov,
                                df = NULL,
                                ...) {
...
#' @export
#' @keywords internal
mymodelparm.glmerMod <- function(model,
                                 coef. = lme4::fixef,
                                 vcov. = vcov,
                                 df = NULL,
                                 ...) {
...
#' @export
#' @keywords internal
mymodelparm.gls <- function(model,
                            coef. = coef,
                            vcov. = vcov,
                            df = NULL,
                            ...) {
...
#' @export
#' @keywords internal
mymodelparm.glmmTMB <- function(model,
                                coef. = glmmTMB::fixef,
                                vcov. = vcov,
                                df = NULL,
                                ...) {
